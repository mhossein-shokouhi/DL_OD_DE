...
def get_nyu_data(batch_size, nyu_data_zipfile='/content/drive/MyDrive/Deep Learning/data.zip'):
    data = extract_zip(nyu_data_zipfile)

    nyu2_train = list((row.split(',') for row in (data['data/data_train.csv']).decode("utf-8").split('\r\n') if len(row) > 0))
    nyu2_test = list((row.split(',') for row in (data['data/data_test.csv']).decode("utf-8").split('\r\n') if len(row) > 0))
...
class NYU_BasicAugmentRGBSequence(Sequence):
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        for i in range(batch_x.shape[0]):
            ...
            x = np.clip(np.asarray(np.load( "/content/drive/MyDrive/Deep Learning/"+sample[0] )).reshape(480,640,3)/255,0,1)
            y = np.clip(np.asarray(np.load( "/content/drive/MyDrive/Deep Learning/"+sample[1] )).reshape(480,640,1),0,self.maxDepth)
...
class NYU_BasicRGBSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):            
            ...
            x = np.clip(np.asarray(np.load( "/content/drive/MyDrive/Deep Learning/"+sample[0])).reshape(480,640,3)/255,0,1)
            y = np.asarray(np.load( "/content/drive/MyDrive/Deep Learning/"+sample[1]), dtype=np.float32).reshape(480,640,1).copy().astype(float)
